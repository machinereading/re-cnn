...
import edu.kaist.mrlab.nn.pcnn.utilities.TestUnit;
import edu.kaist.mrlab.nn.pcnn.utilities.TestUnitComparator;
...
public class Evaluation extends BaseEvaluation<Evaluation> {
	...
	protected int topNTotalCount = 0; // Could use topNCountCorrect /
										// (double)getNumRowCounter() - except
										// for eval(int,int), hence separate
										// counters
	...
	// What to output from the precision/recall function when we encounter an
	// edge case
	protected static final double DEFAULT_EDGE_VALUE = 0.0;

	protected Map<Pair<Integer, Integer>, List<Object>> confusionMatrixMetaData; // Pair:
																					// (Actual,Predicted)
	...
	/**
	 * The labels to include with the evaluation. This constructor can be used
	 * for generating labeled output rather than just numbers for the labels
	 * 
	 * @param labels
	...
	/**
	 * Use a map to generate labels Pass in a label index with the actual label
	 ...
	 *
	 * @param labels
	 *            Labels for the classes (may be null)
	 * @param topN
	 *            Value to use for top N accuracy calculation (<=1: standard
	 *            accuracy). Note that with top N accuracy, an example is
	 *            considered 'correct' if the probability for the true class is
	 *            one of the highest N values
	 */
	...
	/**
	 * Evaluate the output using the given true labels, the input to the multi
	 * layer network and the multi layer network to use for evaluation
	 * 
	 * @param trueLabels
	 *            the labels to ise
	 * @param input
	 *            the input to the network to use for evaluation
	 * @param network
	 *            the network to use for output
	 */
	...
	/**
	 * Collects statistics on the real outcomes vs the guesses. This is for
	 * logistic outcome matrices.
	 * <p>
	 * Note that an IllegalArgumentException is thrown if the two passed in
	 * matrices aren't the same length.
	 *
	 * @param realOutcomes
	 *            the real outcomes (labels - usually binary)
	 * @param guesses
	 ...
	 *            Optional; may be null. If not null, should have size equal to
	 *            the number of outcomes/guesses
	 *
	 */
	...
	public void eval(INDArray realOutcomes, INDArray guesses, List<? extends Serializable> recordMetaData) {
		...
		if (cursor == 0) {
			...
			INDArray temp = guessesAll;
			guessesAll = Nd4j.vstack(temp, guesses);
		}

		// If confusion is null, then Evaluation was instantiated without
		// providing the classes -> infer # classes from
		if (confusion == null) {
		...
		// For each row get the most probable label (column) from prediction and
		// assign as guessMax
		...
		if (nCols > 1 && topN > 1) {
			...
			for (int i = 0; i < nExamples; i++) {
				...
				int countGreaterThan = (int) Nd4j.getExecutioner()
						.exec(new MatchCondition(row, Conditions.greaterThan(prob)), Integer.MAX_VALUE).getDouble(0);
				if (countGreaterThan < topN) {
					// For example, for top 3 accuracy: can have at most 2 other
	 ...
	 *
	 * @param predictedIdx
	 *            Index of class predicted by the network
	 * @param actualIdx
	...
	public void eval(int predictedIdx, int actualIdx) {
		...
		// If confusion is null, then Evaluation is instantiated without
		...
		if (predictedIdx == actualIdx) {
			...
			// And add 1 for each negative class that is accurately predicted
			// (True Negative)
			// (For a particular label)
			...
			// Otherwise the real label is predicted as negative (False
			// Negative)
			incrementFalseNegatives(actualIdx);
			// Otherwise the prediction is predicted as falsely positive (False
			// Positive)
	...
	public void showTopN(int n) {
		...
		for (int i = 0; i < size; i++) {
			...
			INDArray temp = guessesAll.getRow(i);
			INDArray guessScore = temp.max(1);
			INDArray guessIndex = Nd4j.argMax(temp, 1);
			int predictedIdx = (int) guessIndex.getDouble(0);
			double predictedScore = guessScore.getDouble(0);
			String label = labelsList.get(predictedIdx);
			list.add(new TestUnit(sentence, label, predictedScore, sbjPos, objPos));
		}

		TestUnitComparator comparator = new TestUnitComparator();
		Collections.sort(list, comparator);

		for (int i = 0; i < n && i < list.size(); i++) {
			TestUnit tu = list.get(i);
			System.out.println(tu.getSentence() + "\t" + tu.getLabel() + "\t" + tu.getScore() + "\t" + tu.getSbjPos()
					+ "\t" + tu.getObjPos());
		}
	 ...
	 *            whether or not to output warnings related to the evaluation
	 *            results
	 * @return A (multi-line) String with accuracy, precision, recall, f1 score
	 *         etc
	 */
	...
	/**
	 * Precision based on guesses so far Takes into account all known classes
	 * and outputs average precision across all of them
	 *
	...
	/**
	 * Recall based on guesses so far Takes into account all known classes and
	 * outputs average recall across all of them
	 *
	...
	/**
	 * False positive rate based on guesses so far Takes into account all known
	 * classes and outputs average fpr across all of them
	 *
	...
	/**
	 * False negative rate based on guesses so far Takes into account all known
	 * classes and outputs average fnr across all of them
	 *
	...
	/**
	 * False Alarm Rate (FAR) reflects rate of misclassified to classified
	 * records http://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1058&context=isw
	 *
	 * @return the fpr for the outcomes
	 */
	...
	/**
	 * TP: true positive FP: False Positive FN: False Negative F1 score: 2 * TP
	 * / (2TP + FP + FN)
	 *
	...
	public double accuracy() {
		// Accuracy: sum the counts on the diagonal of the confusion matrix,
		// divide by total
	...
	/**
	 * Top N accuracy of the predictions so far. For top N = 1 (default),
	 * equivalent to {@link #accuracy()}
	 * 
	...
	/**
	 * Return the number of correct predictions according to top N value. For
	 * top N = 1 (default) this is equivalent to the number of correct
	 * predictions
	 * 
	...
	/**
	 * Return the total number of top N evaluations. Most of the time, this is
	 * exactly equal to {@link #getNumRowCounter()}, but may differ in the case
	 * of using {@link #eval(int, int)} as top N accuracy cannot be calculated
	 * in that case (i.e., requires the full probability distribution, not just
	 * predicted/actual indices)
	 * 
	...
	/**
	 * Merge the other evaluation object into this one. The result is that this
	 * Evaluation instance contains the counts etc from both
	 *
	 * @param other
	...
	public void merge(Evaluation other) {
		...
		if (topN != other.topN) {
			log.warn(
					"Different topN values ({} vs {}) detected during Evaluation merging. Top N accuracy may not be accurate.",
					topN, other.topN);
		}
	 ...
	 * <p>
	 * <b>Note</b>: Prediction errors are ONLY available if the "evaluate with
	 * metadata" method is used: {@link #eval(INDArray, INDArray, List)}
	 * Otherwise (if the metadata hasn't been recorded via that previously
	 * mentioned eval method), there is no value in splitting each prediction
	 * out into a separate Prediction object - instead, use the confusion matrix
	 * to get the counts, via {@link #getConfusionMatrix()}
	 *
	 * @return A list of prediction errors, or null if no metadata has been
	 *         recorded
	 */
	public List<Prediction> getPredictionErrors() {
		...
		List<Map.Entry<Pair<Integer, Integer>, List<Object>>> sorted = new ArrayList<>(
				confusionMatrixMetaData.entrySet());
	...
	/**
	 * Get a list of predictions, for all data with the specified <i>actual</i>
	 ...
	 *
	 * @param actualClass
	 *            Actual class to get predictions for
	 * @return List of predictions, or null if the "evaluate with metadata"
	 *         method was not used
	 */
	public List<Prediction> getPredictionsByActualClass(int actualClass) {
		...
		for (Map.Entry<Pair<Integer, Integer>, List<Object>> entry : confusionMatrixMetaData.entrySet()) { // Entry
																											// Pair:
																											// (Actual,Predicted)
	...
	/**
	 * Get a list of predictions, for all data with the specified
	 * <i>predicted</i> class, regardless of the actual data class.
	 * <p>
	 * <b>Note</b>: Prediction errors are ONLY available if the "evaluate with
	 * metadata" method is used: {@link #eval(INDArray, INDArray, List)}
	 * Otherwise (if the metadata hasn't been recorded via that previously
	...
	/**
	 * Get a list of predictions in the specified confusion matrix entry (i.e.,
	 * for the given actua/predicted class pair)
	 *
	 * @param actualClass
	 *            Actual class
	 * @param predictedClass
	 *            Predicted class
	 * @return List of predictions that match the specified actual/predicted
	 *         classes, or null if the "evaluate with metadata" method was not
	 *         used
	 */
