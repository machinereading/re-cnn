package edu.kaist.mrlab.nn.pcnn.layers.pooling;
...
import edu.kaist.mrlab.nn.pcnn.iterator.CursorPointor;
...
public class PiecewiseMaxPoolingLayer extends BaseLayer<edu.kaist.mrlab.nn.pcnn.conf.layers.PiecewiseMaxPoolingLayer> {
...
//	private int cursor = 0;
	...
	private CursorPointor cp;

	public PiecewiseMaxPoolingLayer(NeuralNetConfiguration conf) {
		super(conf);

		edu.kaist.mrlab.nn.pcnn.conf.layers.PiecewiseMaxPoolingLayer layerConf = (edu.kaist.mrlab.nn.pcnn.conf.layers.PiecewiseMaxPoolingLayer) conf
				.getLayer();
...
//			cursor = 0;
		...
		if (maskArray == null) {
			...
			List<Integer> sbjPosBatch;
			List<Integer> objPosBatch;
			
			if(cp.getCursor() + GlobalVariables.batchSize > Learner.positionStore.getCursor()){
				sbjPosBatch = new ArrayList<>(Learner.positionStore.getSbjPos().subList(cp.getCursor(), Learner.positionStore.getCursor()));
			} else {
				sbjPosBatch = new ArrayList<>(Learner.positionStore.getSbjPos().subList(cp.getCursor(), cp.getCursor() + GlobalVariables.batchSize));
			}
			
			if(cp.getCursor() + GlobalVariables.batchSize > Learner.positionStore.getCursor()){
				objPosBatch = new ArrayList<>(Learner.positionStore.getObjPos().subList(cp.getCursor(), Learner.positionStore.getCursor()));
			} else {
				objPosBatch = new ArrayList<>(Learner.positionStore.getObjPos().subList(cp.getCursor(), cp.getCursor() + GlobalVariables.batchSize));
			}

			reduced2d = activateHelperFullArray(input, poolDim, sbjPosBatch, objPosBatch);
		} else {
			...
			if (input.rank() == 3) {
				...
				// Masked convolutions. 4d convolution data, shape [minibatch,
				// depth, h, w]
				// and 2d mask array.
				// Because of this: for now we'll support *masked* CNN global
				// pooling on either
				...
				if (maskArray.rank() != 2) {
					throw new UnsupportedOperationException(
							"Only 2d mask arrays are currently supported for masked global reductions "
									+ "on CNN data. Got 4d activations array (shape " + Arrays.toString(input.shape())
									+ ") and " + maskArray.rank() + "d mask array (shape "
				...
				if ((h != 1 && w != 1) || (h != maskLength && w != maskLength)) {
					throw new UnsupportedOperationException(
							"Masked global pooling with on CNN data currently only supports data with h=1 or w=1:\n"
									...
									+ " Got 4d activations array (shape " + Arrays.toString(input.shape()) + ") and "
									+ maskArray.rank() + "d mask array (shape " + Arrays.toString(maskArray.shape())
									+ ")");
				}

				// Valid combinations of global pooling + masking for CNNs:
				// dimensions [2,3] with or without reduction
				...
				boolean maskAlongHeight = (h == maskLength); // At this point:
																// can't confuse
																// w and h, as
																// one has to be
																// 1...
				...
				reduced2d = edu.kaist.mrlab.nn.pcnn.utilities.MaskedReductionUtil.piecewisePoolingConvolution(poolingType,
						input, maskArray, maskAlongHeight, sbjPosBatch, objPosBatch);
			} else {
				throw new UnsupportedOperationException("Invalid input: is rank " + input.rank());
			}
		}

//		cursor += GlobalVariables.batchSize;
		cp.setCursor(cp.getCursor() + GlobalVariables.batchSize);
	...
	private INDArray activateHelperFullArray(INDArray inputArray, int[] poolDim, List<Integer> sbjPos,
			List<Integer> objPos) {
		switch (poolingType) {
		case MAX:
			INDArray result = edu.kaist.mrlab.nn.pcnn.utilities.MaskedReductionUtil
					.piecewisePoolingConvolution(poolingType, input, sbjPos, objPos);
			return result;
		case AVG:
	...
	public Pair<Gradient, INDArray> backpropGradient(INDArray epsilon) {
		...
		if (!collapseDimensions && epsilon.rank() != 2) {
			...
			// Don't collapse dims case: error should be [minibatch, vectorSize,
			// 1] or [minibatch, depth, 1, 1]
		...
		if (maskArray == null) {
			...
			if (input.rank() == 3) {
				...
				epsilonNd = edu.kaist.mrlab.nn.pcnn.utilities.MaskedReductionUtil.piecewisePoolingEpsilonCnn(poolingType,
	...
	private INDArray epsilonHelperFullArray(INDArray inputArray, INDArray epsilon, int[] poolDim) {
		...
		// Broadcast: occurs on the remaining dimensions, after the pool
		...
		case PNORM:
			...
			// dOut/dIn = in .* |in|^(p-2) / ||in||_p^(p-1), where ||in||_p is
			// the output p-norm
	...
	public Pair<INDArray, MaskState> feedForwardMaskArray(INDArray maskArray, MaskState currentMaskState,
			int minibatchSize) {
		// Global pooling layer: no masking is possible after this point...
		// i.e., masks have been taken into account
